...
def forecast_one_area(model: XGBRegressor, df_area: pd.DataFrame, area: str) -> pd.DataFrame:
    ...
    for _ in range(168):
        ...
        row = {
            ...
            "load": df_area["load"].iloc[-24:].mean(),
            "flow_DE": df_area["flow_DE"].iloc[-24:].mean(),
            "flow_SE": df_area["flow_SE"].iloc[-24:].mean(),
            "flow_NO": df_area["flow_NO"].iloc[-24:].mean(),
            "flow_NL": df_area["flow_NL"].iloc[-24:].mean(),
            "hour": new_time.hour,
